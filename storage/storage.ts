...
export const setAppSetupComplete = async () => {
  await AsyncStorage.setItem(APP_SETUP_COMPLETE_KEY, "true");
};

export const isAppSetupComplete = async (): Promise<boolean> => {
  const value = await AsyncStorage.getItem(APP_SETUP_COMPLETE_KEY);
  return value === "true";
};
