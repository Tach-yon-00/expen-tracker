...
const getCurrencySymbol = (): string => {
  try {
    ...
    if (locale.startsWith("de") || locale.startsWith("fr") || locale.startsWith("it")) return "€";
    return "$";
  } catch {
    return "$";
  }
...
export default function TransactionsScreen() {
  ...
  const getCategoryConfigByName = (categoryName: string) => {
    if (!categoryName) return fallbackConfig;
        ...
        onPress: async () => {
          ...
          if (deletedExpense) {
            showToast({
              ...
              type: "undo",
              undoAction: async () => {
                // Handled implicitly by the undo action logic in Toast returning state
              },
              duration: 4000,
            });
          }
